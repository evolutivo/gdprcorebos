...
function showgendoctemplates(module) {
	if(jQuery("#allids").val()=='' && jQuery("#allselectedboxes").val()=='') {
		...
		jQuery.ajax({
			...
			if(document.getElementById("recordval").value=='')  document.getElementById("recordval").value=document.getElementById("allselectedboxes").value;
		});
	}
}

function checkOneTplSelected() {
	if (document.getElementById("gendoctemplate").selectedIndex==-1) {
