...
if($format == 'onepdf'){
	...
	$pdf->concat();
	$pdf->Output(OpenDocument::GENDOCCACHE . '/' . $module . '.pdf', 'F');
	$out.= '<table class="lvtCol" width="100%"><tr><td>';
	$einfo = getEntityName($module, $record);
	$out.=$mod_strings['LBL_INFORMATION'].$modulei18n.' '.$einfo[$record].' '.$mod_strings['LBL_TEMPLATE']." $mergeTemplateName<br/>";
	$out.= '<a href="' . OpenDocument::GENDOCCACHE . '/' . $module . '.pdf">' . $app_strings['DownloadMergeFile'] . '</a></td></tr></table><br/>';
}
$zip->save();
$smarty->assign('THEME', $theme);
$smarty->assign('OUTPUT',$out);
$smarty->assign('ZIPNAME',$zipname);
