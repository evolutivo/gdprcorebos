...
class OpenDocument {
    ...
    private $NS_ENTITIES=array('paragraph','table','table-column','table-cell','table-row','graphic');
    ...
    public function postprocessing($filename,$pFilename=null) {
      ...
      if (preg_match_all('/\\'.$includeGD.'([\w\d]+)\}/', $xmlText, $matches)) {
        foreach($matches[1] as $match) {
          ...
          $sql = "SELECT at.attachmentsid, at.name, at.path FROM vtiger_seattachmentsrel ar LEFT JOIN vtiger_attachments at ON ar.attachmentsid=at.attachmentsid WHERE crmid={$docid}";
          $res = $adb->query( $sql );
        ...
        foreach ($iterat as $child) {
            ...
            if (get_class($child) == 'OpenDocument_TextElement') {
                ...
                foreach ($pnodecld as $pnc) {
                    ...
                    if (!empty($texto_p) && is_string($texto_p)) break; // utilizo el primer texto no vacío que encuentro
                }
                $output .= $texto_p.'<br>';
            }
        }
    ...
    public function toGenDoc($obj,$id,$module,$root_module=null)
    {
        ...
        foreach ($iterat as $child) {
            ...
            $hayqueincluir=(count($siincluir)==0 or $siincluir[count($siincluir)-1]);
            ...
            case 'OpenDocument_Span':
                ...
                if (strtolower(substr($texto_p,0,strlen($ifexistsGD)))==$ifexistsGD) {
                    ...
                    if ($cumple_cond and $hayqueincluir) {
                ...
                if ($hayqueincluir) {
                	if ((get_class($child))=='OpenDocument_Span')
                    	...
                    	$pgsp = $topofarray->createParagraph();
            ...
            case 'OpenDocument_Hyperlink':
            	if ($hayqueincluir) $this->compiletoDoc($child,$id,$module);
            	break;
            ...
            default:
                ...
                if (!is_null($domNode) and get_class($domNode)=='DOMElement')
    ...
    public function applyStyle($style_name, $name, $value, OpenDocument_StyledElement $object, $elemtype='')
    {
        ...
        if (empty($style)) {
            if (empty($style_name)) $style_name = $object->generateStyleName();
...
/*        $ReflectionClass = new ReflectionClass($object);
        ...
        if (empty($elemtype)) $elemtype='text';
    ...
    public function addStyles($node, $elem, $elemtype, $keepname=false)
    {
        ...
        if ($this instanceof OpenDocument) {
            $document = $this;
        } else if ($this instanceof OpenDocument_Element) {
            $document = $this->getDocument();
        }
        if (empty($document->originGenDocStyles)) return 0;
        if (!array_key_exists($style_name,$document->originGenDocStyles)) return 0;
        $stylebranch=$document->originGenDocStyles[$style_name];
        if (!is_array($stylebranch)) return 0;
              ...
              while ($level2style=current($level1stbranch)) {
                if (is_array($level2style)) {
                   ...
                   while ($level3style=current($level2stbranch)) {
                     if (key($level2stbranch)!='name' or (key($level2stbranch)=='name' and $keepname)) $elem->applyStyle(key($level2stbranch),$level3style, $level2et);
                     next($level2stbranch);
                   }
                } else {
                  if (key($level1stbranch)!='name' or (key($level1stbranch)=='name' and $keepname)) $elem->applyStyle(key($level1stbranch),$level2style, $level1et);
                }
                next($level1stbranch);
              }
            } else {
              if (key($stylebranch)!='name' or (key($stylebranch)=='name' and $keepname)) $elem->applyStyle(key($stylebranch),$level1style, $elemtype);
            }
            next($stylebranch);
        }
    }
    ...
    public function addListStyles($node)
    {
        ...
        if (empty($stylename)) return;
        if ($this->hasListStyleNode($stylename)) return; // Si ya lo tenemos, no lo repetimos
    ...
    private function hasListStyleNode($style_name)
    {
        ...
        foreach ($nodes as $node) {
            ...
            for ($i = 0; $i < $attributes->length; $i ++) {
                ...
                if ($name=='style:name' and $value==$style_name) return true;
            }
        }
        return false;
    }
    ...
    public function copyAttributes($orgNode,$destNode,$changedImage='')
    {
        ...
        if (!is_null($attributes)){
            foreach ($attributes as $attrib) {
                ...
                $value = $attrib->value;
                //echo "$aname = $value<br>";
                if ($attrib->name=='href' and $changedImage!='') // tenemos que cambiarlo, sustitución imagen paracada
                  $value='Pictures/'.$changedImage;
                if (!$destNode->getNode()->hasAttribute($aname)) $destNode->getNode()->setAttribute($aname, $value);
            }
        }
        return true;
    }
    ...
    public function getImageNodeName($imgNode)
    {
        ...
        foreach ($attributes as $attrib) {
            if ($attrib->name=='href') return $attrib->value;
        }
        return '';
    }
    ...
    {
        $ret='';
        if (empty($node->attributes)) return $ret;
        $attributes = $node->attributes;
        for ($i = 0; $i < $attributes->length; $i ++) {
            $name = $attributes->item($i)->name;
            $value = $attributes->item($i)->value;
            if ($name == 'style-name' or $name == 'data-style-name') return $value;
        }
        return $ret;
    }
    ...
    public function processHTML($compiledtext,$child){
        global $parentArray;
        $topofarray=$parentArray[count($parentArray)-1];
        if (stripos($compiledtext,'<b>')!==false or
            stripos($compiledtext,'<i>')!==false or
            stripos($compiledtext,'<u>')!==false or
            stripos($compiledtext,'<style')!==false or
            stripos($compiledtext,'<strong')!==false or
            stripos($compiledtext,'<em')!==false or
            stripos($compiledtext,'<gendocstyle')!==false or
            stripos($compiledtext,'<br')!==false or
            stripos($compiledtext,"\n")!==false or
                            stripos($compiledtext,'<div')!==false) { // hay html, traducir a odt
...
//            $this->processHTML('<div>'.$compiledtext.'</div>',$child);
	...
	public function GenXML($originFile,$id,$module,$root_module=null) {
		...
		if(!is_null($root_module)) $iter_modules[$root_module[0]] = array($root_module[1]);
		if(!is_dir('cache/genxml')) mkdir('cache/genxml');
	...
	function processBranch($doc,$crmid,$module) {
		...
		foreach ($doc->childNodes as $node) {
			...
			if (empty($condicion)) $condicion = $entidad;
			switch ($node->tagName) {
				case 'genxmlforeach':
				...
				case 'genxmlifexist':
					...
					if ($cumple_cond and $this->hasChild($node)) {
						eval_paracada($condicion,$crmid,$module);
						$this->processBranch($node,$iter_modules[$entidad][0],$entidad);
					}
					continue 2;
	...
	function startAggregate($action,$entity,$field='') {
		global $genxmlaggregates;
		if (empty($field)) $field="genxml$action";
		$genxmlaggregates[$action][$entity][$field] = 0;
	}

	function accumAggregate($crmid,$module) {
		global $genxmlaggregates;
